...
class _BaseHMM():
    ...
    def __init__(self, n_state=1, x_size=1, iter=20):
        ...
        self.x_size = x_size
        self.start_prob = np.ones(n_state) * (1.0 / n_state)  # 初始状态概率， 每个隐藏状态的可能性
    ...
    def forward(self, X, Z):
        ...
        alpha[0] = alpha[0] / c[0]
        # 递归传递
        for i in range(1, X_length): # 对于每个变量
            # 首先取出观测变量x[i]的每个状态的概率，也就是上一步参数下的发射概率
            # 然后根据转移矩阵求出上一步的状态转移到当前状态的概率
            # 接着乘上当前隐状态的概率
            # 最后得到当前状态
            alpha[i] = self.emit_prob(X[i]) * np.dot(alpha[i - 1], self.transmat_prob) * Z[i]
            # 归一化
            c[i] = np.sum(alpha[i]) # 每个变量各个状态的概率之和
            if c[i]==0: continue
            alpha[i] = alpha[i] / c[i]
...
class DiscreteHMM(_BaseHMM):
    ...
    def emit_prob(self, x):
        ...
        prob = np.zeros(self.n_state)
        # 更新每个状态下观测值的概率，
        # X为观测变量，二维矩阵： np.array([[1],[6],[3],[5],[2],[7],[3],[5],[2],[4],[3],[6],[1],[5],[4]])
        for i in range(self.n_state):
